...
class Handlers:
    ...
    def handle_load(self):
        ...
        print("=== Load Data ===")
        try:
            self.network.load_from_json(RAILWAYS_FILE)
            print(f"Stations loaded from {RAILWAYS_FILE}")
            self.sim.load_trains_from_json(TRAINS_FILE)
    ...
    def add_station(self):
        ...
        try:
            station = self.network.create_station(name)
            print(f"Station '{station.name}' added.")
            self.network.save_to_json(RAILWAYS_FILE)
    ...
    def add_track(self):
        ...
        try:
            ...
            self.network.create_track(from_name, to_name, distance, max_speed)
    ...
    def add_train(self):
        ...
        try:
            ...
            self.sim.save_trains_to_json(TRAINS_FILE)
        except (ValueError, NotFoundError) as e:
            print(f"Error: {e}")
        pause()
    ...
    def add_stop(self):
        ...
        try:
            ...
            station = self.network.get_station(station_name)
            stop = Stop(station=station, arrival_time=arrival, departure_time=departure)
            self.sim.add_stop_to_train(train_id, stop)
    ...
    def add_carriage_to_train(self):
        ...
        train = self.sim.get_train(train_id)
    ...
    def rename_station(self):
        ...
        name = pick_from_list("station", list(self.network.stations.keys()))
        if not name:
            return

        try:
            new_name = input("New name: ").strip()
            self.network.rename_station(name, new_name)
    ...
    def remove_station(self):
        ...
        try:
            # Remove trains using this station - easier since pathfinding would be too time-consuming for this project
            removed_trains = self.sim.remove_trains_using_station(name)
            if removed_trains:
                print(f"  Removed trains: {', '.join(removed_trains)}")

            self.network.remove_station(name)
    ...
    def remove_track(self):
        ...
        try:
            ...
            removed_trains = self.sim.remove_trains_using_track(from_id, to_id)
            if removed_trains:
                print(f"  Removed trains: {', '.join(removed_trains)}")

            self.network.remove_track(from_id, to_id)
    ...
    def remove_train(self):
        ...
        train_id = pick_from_list("train", list(self.sim.trains.keys()))
        if not train_id:
            return

        try:
            self.sim.remove_train(train_id)
    ...
    def handle_search(self):
        ...
        direct_only = direct_input.lower() == "y"
        ...
        try:
            journeys = self.sim.search_journeys(
