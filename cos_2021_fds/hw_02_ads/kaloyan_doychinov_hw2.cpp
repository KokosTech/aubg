...
void removeAndPrintQueue(Queue<std::string> &queue) {
    std::optional<std::string> cur = queue.Remove();
    while (cur && cur.has_value()) {
        std::cout << cur.value() << " ";
        cur = queue.Remove();
    }
...
int main() {
    ...
    while (cur && cur.has_value() && !cur.value().empty()) {
        std::string cur_val = cur.value();

        if (cur_val == "(") {
            std::shared_ptr<Queue<std::string>> new_word_queue = std::make_shared<Queue<std::string>>();
